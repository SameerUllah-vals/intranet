    ...
    public class NewsLetterController : BaseController
    {
        ...
        public JsonResult Listener(DTParameters param)
        {
            ...
            var resultData = from x in resultList
                             select new { x.ID, x.Title, x.Image, x.PDF, x.isDefault, x.Status, CreatedBy = x.User1.Name, UpdatedBy = (x.User != null ? x.User.Name : ""), CreatedDateTime = x.CreatedDateTime.ToString(Website_Date_Time_Format), UpdatedDateTime = (x.UpdatedDateTime.HasValue ? x.UpdatedDateTime.Value.ToString(Website_Date_Time_Format) : "") };
            var result = new
            {
                draw = param.Draw,
                data = resultData,
