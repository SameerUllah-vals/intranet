...
public class RoxyFilemanHandler : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    string rootMainFolder;
    string completePath;

    public void ProcessRequest (HttpContext context) {

        _context = context;
        var policytype = GetCookie("policy");
        switch (policytype.ToLower())
        {
            case "engineering":
                completePath = ConfigurationManager.AppSettings["EngineeringPoliciesPath"];
                break;
            case "power":
                completePath = ConfigurationManager.AppSettings["PowerPolicyPath"];
                break;
            case "inspect":
                completePath = ConfigurationManager.AppSettings["InspectPoliciesPath"];
                break;
            case "chemical":
                completePath = ConfigurationManager.AppSettings["ChemicalPoliciesPath"];
                break;
            case "sap":
                completePath = ConfigurationManager.AppSettings["SAPTrainingPath"];
                break;
            default:
                completePath = "";
                break;
        ...
        confFile = "../conf.json";
        var arr = completePath.Split('\\');
        for (int i = 0; i < arr.Length - 1; i++)
        {
            if(i == (arr.Length - 2))
                rootDirectoryPath += arr[i];
            else
                rootDirectoryPath += arr[i] + "\\";
        }
        rootMainFolder = "\\" + completePath.Split('\\')[completePath.Split('\\').Length - 1];
        _r = context.Response;
    ...
    protected void CopyDir(string path, string newPath)
    {
        CheckPath(path);
        CheckPath(newPath);
        DirectoryInfo dir = new  DirectoryInfo(rootDirectoryPath + FixPath(path));
        DirectoryInfo newDir = new DirectoryInfo(rootDirectoryPath + FixPath(newPath + "/" + dir.Name));
    ...
    {
        CheckPath(path);
        FileInfo file = new FileInfo(rootDirectoryPath + FixPath(path));
        newPath = rootDirectoryPath + FixPath(newPath);
        if (!file.Exists)
    ...
    {
        CheckPath(path);
        path = rootDirectoryPath + FixPath(path);
    ...
    protected void MoveDir(string path, string newPath)
    {
        CheckPath(path);
        CheckPath(newPath);
        DirectoryInfo source = new DirectoryInfo(rootDirectoryPath + FixPath(path));
        DirectoryInfo dest = new DirectoryInfo(rootDirectoryPath + FixPath(Path.Combine(newPath, source.Name)));
    ...
    protected void MoveFile(string path, string newPath)
    {
        CheckPath(path);
        CheckPath(newPath);
        FileInfo source = new FileInfo(rootDirectoryPath + FixPath(path));
        FileInfo dest = new FileInfo(rootDirectoryPath + FixPath(newPath));
    ...
    protected void RenameDir(string path, string name)
    {
        ...
        {
            throw new Exception("Not able to rename main root folder");
        }
    }
    ...
    {
        CheckPath(path);
        FileStream fs = new FileStream(rootDirectoryPath + FixPath(path), FileMode.Open, FileAccess.Read);
